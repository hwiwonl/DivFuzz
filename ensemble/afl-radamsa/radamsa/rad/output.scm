...
(define-library (rad output)
   ...
   (begin
      ...
      ;; output :: (ll' ++ (#(rs mutator meta))) fd → rs mutator meta (n-written | #false), handles port closing &/| flushing
      (define (output ll fd)
         (lets 
      ...
      (define (dummy-output ll)
        (lets
          ((state (lfold (λ (prev this) this) #false ll))
           (rs muta meta state))
          (values rs muta meta)))
